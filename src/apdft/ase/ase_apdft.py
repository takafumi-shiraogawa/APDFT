...
class mod_APDFT(FileIOCalculator):
  ...
  # For MPI
  # num_mpi_proc = 50
  # command = "( cd work/temp && bash imp_mod_cli1.sh && pre_MPI_div_QM.py %s && mpiexec -np %s python3 ~/my_code/APDFT/src/MPI_div_QM.py && bash imp_mod_cli2.sh )" % (str(num_mpi_proc), str(num_mpi_proc))
    ...
    for i in range(num_atoms):
      for didx, dim in enumerate("xyz"):
        try:
          ...
          raise FileNotFoundError("ver_atomic_force_%s_%s_order1 can not be found." % (str(i), dim))
        except KeyError:
